...
class Grid:
	def __init__(self, dimensions=20, maxTunnels=100, maxLength=5):
		...
		self.directions = [[0, -1], [1, 0], [0, 1], [-1, 0]] # top, right, bottom, left as [y, x] coordinates
		...
		for y in range(0, self.dimensions):
			for x in range(0, self.dimensions):
				...
				if not self.grid[y][x] == 0:
					...
					self.grid[y][x] = Room(title=f"Room {y}, {x}")
				...
				if not self.grid[y][x] == 0:
					...
					for i in range(0, len(neighbors)):
						if neighbors[i] is not None:
							...
							self.grid[y][x].connectRooms(neighbors[i], self.getDirection(i))
